...
- `category` controls the visible grouping:
  - Original and United Forever race-only data is grouped under `Race`.
...
- The atlas still includes only the race campaign data we have names for in several legacy games besides Sunrise. It no longer invents top-level categories for absent Puzzle, Platform, Stunt, Survival, or Crazy campaign data.
